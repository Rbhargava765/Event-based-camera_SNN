# This file is a patched copy of OF_EV_SNN-main/network_3d/poolingNet_cat_1res.py
...
# Update import to use activation_based instead of clock_driven
from spikingjelly.activation_based import neuron

# Changed from relative import to absolute import
from network_3d_patched.blocks import SeparableSEWResBlock
...
class NeuronPool_Separable_Pool3d(nn.Module):
    ...
    def __init__(self, multiply_factor: float = 10.):
        ...
        self.deconv4 = separable_decoder_block(in_channels = 512, out_channels = 256, up_size = (60, 80),  kernel_size = 7, multiply_factor = multiply_factor)
        self.deconv3 = separable_decoder_block(in_channels = 256 * 2, out_channels = 128, up_size = (120, 160),  kernel_size = 7, multiply_factor = multiply_factor)
        self.deconv2 = separable_decoder_block(in_channels = 128 * 2, out_channels = 64, up_size = (240, 320),  kernel_size = 7, multiply_factor = multiply_factor)
        self.deconv1 = separable_decoder_block(in_channels = 64 * 2, out_channels = 32, up_size = (480, 640),  kernel_size = 7, multiply_factor = multiply_factor)

        # Predictor Block


        self.pred4 = separable_predictor_block(in_channels = 256 * 2, out_channels = 2, up_size = (480, 640), kernel_size = 7)
        self.pred3 = separable_predictor_block(in_channels = 128 * 2, out_channels = 2, up_size = (480, 640), kernel_size = 7)
        self.pred2 = separable_predictor_block(in_channels = 64 * 2, out_channels = 2, up_size = (480, 640), kernel_size = 7)
        self.pred1 = separable_predictor_block(in_channels = 32 * 2, out_channels = 2, up_size = (480, 640), kernel_size = 7)
    ...
    def forward(self, x):
        ...
        up_1 = self.pred1(torch.cat((out_deconv1, in_conv1[:, :, -1]), axis = 1))
...
if __name__ == '__main__':
    ...
    in_tensor = torch.randn(1, 2, 21, 480, 640)
    net = NeuronPool_Separable_Pool3d()
    _, _, _, out_tensor = net(in_tensor)
